...
print(f"Factoring degree {outpoly.degree()} polynomial")

for target_degree in range(k,0,-1):
    print("invoking SS")
    for solution_indices in find_subset_sum(terms, target_degree):
...
'''
with open(f'systems/facto-DSA-{p}-{2*n-1}-{2*n}.txt','w') as f:
    f.write(magma_code)
...
time GB:=GroebnerBasis(I);
'''
with open(f'systems/facto-DSA-feqs-{p}-{2*n-1}-{2*n}.txt','w') as f:
    f.write(magma_code)